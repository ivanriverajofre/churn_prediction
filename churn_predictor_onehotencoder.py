...
def mapTextColumn(data,columnas):
  ...
  if (not savedEnc):
    ...
    colT = compose.make_column_transformer((encoder, columnas), remainder='passthrough')
...
def preprocessData(data_src):
  ...
  data = mapTextColumn(data,['sexo','estudios','actividad','comuna','plan'])

  # Normalizo
  data[0] = normalize(data,0) #fnac
  data[1] = normalize(data,1) #recl_3
  data[2] = normalize(data,2) #recl_12
